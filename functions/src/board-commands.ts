...
const getEmptyBoard = () => {
  ...
  for (let j = 8; j >= 1; j--) {
    ...
    fileKeys.forEach((currentFileKey: string) => {
      const k = i + j;
      initialBoard.push({
        color: k % 2 === 0 ? Team.white : Team.black,
        rank: j,
        file: currentFileKey,
      });
      i++;
...
const movePiece = (
  ...
  console.log(
    `${loggingPrefix}${functionPrefix}
    request to move ${fromFile}${fromRank} to ${toFile}${toRank}`
  );
  ...
  if (isCapturingSelf) {
    console.log(
      `${loggingPrefix}${functionPrefix}
      No piece can capture a piece on the same team`
    );
    return undefined;
  }

  if (
    pieceToMove.description === PieceDescription.pawn &&
    !isLegalPawnMove(squareToMoveFrom, squareToMoveTo)
  ) {
    return undefined;
  }
  if (
    pieceToMove.description === PieceDescription.knight &&
    !isLegalKnightMove(squareToMoveFrom, squareToMoveTo)
  ) {
    return undefined;
  }
  if (
    pieceToMove.description === PieceDescription.bishop &&
    !isLegalBishopMove(squareToMoveFrom, squareToMoveTo)
  ) {
    return undefined;
  }
  if (
    pieceToMove.description === PieceDescription.king &&
    !isLegalKingMove(squareToMoveFrom, squareToMoveTo)
  ) {
    return undefined;
  }
...
const placePiecesOnNewBoard = (board: Board) => {
  ...
  pieces.forEach((piece) => {
    const foundStartSquare = getPieceStartingSquare(piece, board);
    if (foundStartSquare) {
...
const numToFile = (x: number) => {
  ...
  if (charCode < 97 || charCode > 104) {
    throw new Error("numToFile: file indxing is out of whack");
  }
...
const fileToNum = (file: string) => {
  ...
  if (num < 0 || num > 7) {
    throw new Error("fileToNum: file indexing is out of whack");
  }
  return num;
};
...
const getPieceStartingSquare = (piece: Piece, board: Board) => {
  return board.find(
    (square) =>
      square.rank === piece.startRank && square.file === piece.startFile
  );
};
const getSquare = (
  file: string,
  ...
  return board.find((square) => square.file === file && square.rank === rank);
};
const moveIsBlocked = (
  squareToMoveFrom: Square,
  squareToMoveTo: Square,
  board: Board
) => {
  functionPrefix = "isMoveBlocked -- ";
  if (!squareToMoveFrom.piece) {
    throw new Error("squareToMoveFrom must have a piece in it");
  }
  ...
  const [startRank, startFileX] = [
    squareToMoveFrom.rank,
    fileToNum(squareToMoveFrom.file),
  ];
  const [endRank, endFileX] = [
    squareToMoveTo.rank,
    fileToNum(squareToMoveTo.file),
  ];
...
const isMoveTeamTurn = (moveTeam: Team, currentTurnTeam: Team) => {
  ...
  const result = moveTeam === currentTurnTeam;
  if (!result) {
    console.log(
      `${loggingPrefix}${functionPrefix}It is not ${moveTeam}'s turn.`
    );
  }
  return result;
};
...
const isLegalKingMove = (squareToMoveFrom: Square, squareToMoveTo: Square) => {
  ...
  if (squareToMoveFrom.piece.description !== PieceDescription.king) {
    throw new Error(
      "do not call isLegalKingMove for a piece that is not a king"
    );
  }

  const rankDiff = Math.abs(getRankDiff(squareToMoveFrom, squareToMoveTo));
  const fileDiff = Math.abs(
    getFileDiff(squareToMoveFrom.file, squareToMoveTo.file)
  );
  ...
  if (!kingMoveIsOneSpaceOnly) {
    console.log(
      `${loggingPrefix}${functionPrefix}
      king can move only one space in any direction`
    );
    return false;
  }
...
const isLegalBishopMove = (
  ...
  if (!squareToMoveFrom.piece) {
    throw Error("squareToMoveFrom must have a piece");
  }

  if (squareToMoveFrom.piece.description !== PieceDescription.bishop) {
    throw new Error(
      "do not call isLegalBishopMove for a piece that is not a pawn"
    );
  }
...
const isLegalKnightMove = (
  ...
  if (squareToMoveFrom.piece.description !== PieceDescription.knight) {
    throw new Error(
      "do not call isLegalKnightMove for a piece that is not a pawn"
    );
  }
...
const isLegalPawnMove = (squareToMoveFrom: Square, squareToMoveTo: Square) => {
  ...
  if (squareToMoveFrom.piece.description !== PieceDescription.pawn) {
    throw new Error(
      "do not call isLegalPawnMove for a piece that is not a pawn"
    );
  }
  ...
  if (rankDiff > 2) {
    console.log(
      `${loggingPrefix}${functionPrefix}pawn cannot move more than 2 spaces`
    );
    return false;
  }

  const pawnMoveIsFromStartRank =
    squareToMoveFrom.rank === squareToMoveFrom.piece.startRank;

  if (rankDiff === 2 && !pawnMoveIsFromStartRank) {
    console.log(
      `${loggingPrefix}${functionPrefix}
      pawn can only make 2-space-move from starting position`
    );
    return false;
  }
  ...
  if (fileDiff === 0 && !!squareToMoveTo.piece) {
    console.log(
      `${loggingPrefix}${functionPrefix}
      Pawn may only capture on the diagonal 1`
    );
    return false;
  }
  ...
  if (fileDiff === 1 && !squareToMoveTo.piece) {
    console.log(
      `${loggingPrefix}${functionPrefix}
      Pawn must move forward or capture diagonally`
    );
    return false;
  }
  // TODO -- EN PASSENT
...
// const isKingMoveTargetUnderAttack =
// (targetSquare: { file: any; rank: number; }, kingColor: any) => {
//     // if (!square.piece) throw Error('no piece to test if attacked')
//     // if (square.piece.description !== king) throw Error('do not call
// isKingTargetUnderAttack for a square that does not have a king')
